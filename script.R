...
# nobody uses numbers in place of months, so here are the names for later use as lables

png("plot_nidificazione.png", width = 600, height = 500)     

# let's save it as a png, specifying desired dimentions 
...
ggplot(sdfr, aes(value, nomi)) +                                                                  # i want to plot sdfr, using the species names
  ...
  xlab(NULL) + 
  ylab(NULL) +
  theme(plot.title = element_text(size=18, face="bold",hjust = 0.3),                              # title size, style and position
